...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        distance = random.gauss(motion[1],distance_noise)
        steering_angle = random.gauss(motion[0],steering_noise)
    ...
    def sense(self, flag): #do not change the name of this function
        ...
        if flag == 1:
            for i in range(len(landmarks)):
                delta_y = random.gauss(landmarks[i][0]-self.y,bearing_noise)
                delta_x = random.gauss(landmarks[i][1]-self.x,bearing_noise)
